...
test.describe("Program Management -> Class", () => {
    ...
    test("Whitespace only in class name", async ({page}) => {
        ...
        await expect(page.getByText("Class name can not be empty or just spaces.")).toBeVisible();
    });
    ...
    test("Admin cannot create a class with a duplicate name", async ({page}) => {
        const createClass = new Class(page);
        ...
        await expect(page.getByText("Class already exists with this name")).toBeVisible();
    });
    ...
    test("Admin can select multiple class managers", async ({page}) => {
        ...
        await page.waitForSelector(".ant-select-dropdown .rc-virtual-list-holder-inner", {visible: true});
